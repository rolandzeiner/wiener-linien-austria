...
// Generated by `scripts/build-mdi-icon-names.mjs` from @mdi/js's
// declaration file at build time. ~7,500 kebab-case `mdi:*` strings —
// the entire MDI catalog bundled in source name form (no SVG paths),
// so the "Extra icons" chip input can autocomplete against the full set.
// One-time cost (~165 KB raw, ~40 KB after terser+gzip).
import mdiIconNames from "./mdi-icon-names.json";

/** Module-level memoised `{value,label}[]` for the MDI chip-input
 *  `options:`. Built once at first reference (not per render or per
 *  editor instance) so the ~7.5k-entry array isn't re-allocated on
 *  every form change. Both `value` and `label` are the MDI key
  ...
  private _schema(): ReadonlyArray<HaFormSchema> {
    ...
    return [
      ...
      },
      {
        ...
        flatten: true,
        schema: [
          // Master toggle for the entire header strip — first entry
          // in the section so the user sees it immediately on expand.
          // Default off; per-side configs stay intact when toggled.
          { name: "show_header", selector: { boolean: {} } },
          {
            ...
            schema: [
              ...
              // webawesome popover library whose click-commit doesn't
              // round-trip reliably when nested inside a
              // `flatten: false` expandable two levels deep. HA core
              // avoids this by keeping icon selectors at the root data
              // level or inside `flatten: true` expandables.
              // We need per-side namespacing, so we can't switch to
              // `flatten: true` — instead, this falls back to the
              // proven `select + multiple + custom_value` chip pattern
              // (same as the text `chips` field above): user types
              // "mdi:foo" + Enter, repeats. No icon picker UI but the
              // value commits reliably. Normaliser drops anything
              // without the `mdi:` prefix.
              {
                name: "extra_icons",
                selector: {
                  select: {
                    multiple: true,
                    custom_value: true,
                    // Full MDI catalog as suggestions — user can pick
                    // from the autocomplete list OR type a custom key
                    // (custom_value: true). Normaliser still drops
                    // anything without the `mdi:` prefix.
