...
- **S-Bahn on departure boards** *(2.0.0)* — track a stop's S-Bahn lines next to its Wiener Linien lines. They show timetable times only, since no live data exists for them, and every card marks those rows *Timetable only*. On the modern card, an S-Bahn departure expands into its stops ahead like any other, with the Wiener Linien lines you can change to.
